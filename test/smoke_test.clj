...
(defn- with-deps-edn
  "Writes `contents` to a deps.edn in a temp dir and hands the path to `f`."
  [contents f]
  (let [dir (str (fs/create-temp-dir))
        path (str (fs/path dir "deps.edn"))]
    (try
      (spit path (if (string? contents) contents (pr-str contents)))
      (f path)
      (finally (fs/delete-tree dir)))))

(deftest test-aliases-with-extra-deps
  (testing "aliases contributing :extra-deps are found, sorted"
    (with-deps-edn
      '{:deps {org.clojure/clojure {:mvn/version "1.12.0"}}
        :aliases {:test {:extra-deps {lambdaisland/kaocha {:mvn/version "1.0"}}}
                  :app {:extra-deps {at.yawk.lz4/lz4-java {:mvn/version "1.11.3"}}}}}
      #(is (= [:app :test] (aliases-with-extra-deps %)))))

  (testing "aliases without :extra-deps are ignored"
    (with-deps-edn
      '{:deps {org.clojure/clojure {:mvn/version "1.12.0"}}
        :aliases {:build {:extra-paths ["build"]}
                  :run {:main-opts ["-m" "app.core"]}
                  :empty {:extra-deps {}}}}
      #(is (= [] (aliases-with-extra-deps %)))))

  (testing "a deps.edn with no aliases at all"
    (with-deps-edn
      '{:deps {org.clojure/clojure {:mvn/version "1.12.0"}}}
      #(is (= [] (aliases-with-extra-deps %)))))

  (testing "an unknown tagged literal does not abort the scan"
    ;; written as text on purpose - as a literal it would break this file's
    ;; own reader long before the test could run
    (with-deps-edn
      (str "{:deps {org.clojure/clojure {:mvn/version \"1.12.0\"}}\n"
           " :aliases {:app {:extra-deps {some/lib {:mvn/version \"1.0\"}}}\n"
           "           :weird {:some-key #some/tag \"value\"}}}")
      #(is (= [:app] (aliases-with-extra-deps %))))))
