...
(def bb-edn-path (-> *file* fs/parent (fs/path "bb.edn") str))

(def pom-generator-dir
  (-> *file* fs/parent (fs/path "pom-generator") str))

(def antq-deps
  "EDN for `clojure -Sdeps`, pinning antq to the version bb.edn already uses.

  This used to pass the *path* to bb.edn. The CLI wants an EDN map, not a
  filename, so it exited with \"Invalid token\" and every antq call failed."
  (-> bb-edn-path
      slurp
      edn/read-string
      (update :deps select-keys ['com.github.liquidz/antq])
      (select-keys [:deps])
      pr-str))
...
(defn- configure-git [github-workspace]
  (when-not (System/getenv "LOCAL_DEV")
    (log "Configuring git")
    (sh "git" "config" "--global" "user.email" "41898282+github-actions[bot]@users.noreply.github.com")
...
(defn- configure-ssh []
  (when-let [private-key (System/getenv "SSH_PRIVATE_KEY")]
    (log "Configuring SSH")
    ;; deps.edn may point at private repositories over ssh. Resolution has to
    ;; fetch those before it can report anything about them, so without a key
    ;; the whole scan fails rather than just missing a dependency.
    (let [ssh-dir (fs/path (System/getProperty "user.home") ".ssh")
          key-file (fs/path ssh-dir "id_rsa")
          config-file (fs/path ssh-dir "config")]
      (fs/create-dirs ssh-dir)
      ;; ssh rejects a key with loose permissions or no trailing newline
      (spit (str key-file) (str (str/trim private-key) "\n"))
      (fs/set-posix-file-permissions key-file "rw-------")
      (spit (str (fs/path ssh-dir "known_hosts"))
            (:out (sh "ssh-keyscan" "github.com"))
            :append true)
      (spit (str config-file)
            (str "Host github.com\n"
                 "  IdentityFile " key-file "\n"
                 "  StrictHostKeyChecking accept-new\n"))
      (fs/set-posix-file-permissions config-file "rw-------"))))
...
(defn aliases-with-extra-deps
  "Every alias in a deps.edn that contributes :extra-deps.

  Dependencies declared under an alias are real dependencies - they ship, and
  they carry vulnerabilities - but nothing in deps.edn marks which aliases
  matter. Resolving without them silently drops the lot, so take them all: an
  alert for a dev dependency is a better failure than a missing one."
  [deps-edn-path]
  ;; :default keeps an unfamiliar tagged literal in somebody's deps.edn from
  ;; aborting the whole scan - we only care about the shape of :aliases here.
  (->> (:aliases (edn/read-string {:default (fn [_tag value] value)}
                                  (slurp deps-edn-path)))
       (keep (fn [[alias-name alias-map]]
               (when (seq (:extra-deps alias-map)) alias-name)))
       sort
       vec))

(defn- write-trace
  "Resolves `dir` with tools.deps, returning the path to the trace it wrote."
  [dir]
  (let [aliases (aliases-with-extra-deps (str (fs/path dir "deps.edn")))]
    (log "Resolving" dir
         (if (seq aliases)
           (str "with aliases " (str/join " " aliases))
           "with no aliases"))
    (apply sh {:dir dir}
           (cond-> ["clojure"]
             (seq aliases) (conj (str "-A" (str/join aliases)))
             :always (conj "-Strace")))
    (str (fs/path dir "trace.edn"))))

(defn- deps-edn-pom
  "Writes pom.xml for a deps.edn project from a tools.deps resolution.

  `clojure -X:deps mvn-pom` would be the obvious call, but it emits only the
  top-level :deps and leaves transitive versions to Maven, which resolves
  differently from tools.deps. See pom-generator/pom_generator.clj."
  [dir github-repository]
  (let [trace-path (write-trace dir)
        project (-> github-repository (str/split #"/") last)]
    (try
      (sh {:dir pom-generator-dir}
          "clojure" "-X" "pom-generator/generate-pom"
          ":repository" (pr-str project)
          ":trace-path" (pr-str trace-path)
          ":destination" (pr-str (str (fs/path dir "pom.xml"))))
      (finally
        (fs/delete-if-exists trace-path)))))
  ...
  (doseq [{:keys [full-path project-path]} files]
    (let [file-name (fs/file-name project-path)
          pom-gen (if (= file-name "deps.edn")
                    #(deps-edn-pom % github-repository)
                    #(sh {:dir %} "lein" "pom"))]
...
(defn- vuln-summary [file alerts outdated]
  ...
  (let [dep-tree (-> file :full-path fs/parent str mvn-tree)
        filtered-alerts (filter
                          (fn [alert] (= (-> alert :dependency :manifest-path)
                                         (:project-path file)))
                          alerts)]
    (str
              ...
              "\n"
               [:tbody (for [alert filtered-alerts]
                         [:tr
                          ...
                          [:td (-> alert :dependency :package :name)]
...
(defn- outdated-cmd [dir]
  (proc/sh {:dir dir}
           "clojure" "-Sdeps" antq-deps "-M" "-m" "antq.core" "--reporter=json"))
...
(defn- open-pr
  ...
  ;; Every element of this concat must be a collection. The title and body
  ;; flags used to sit here as bare strings, which concat tore into single
  ;; characters, so gh received "-" "-" "t" "i" "t" "l" "e" ...
